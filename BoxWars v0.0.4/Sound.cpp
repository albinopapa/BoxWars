...
Sound DSound::CreateSound( char* wavFileName )
{
	...
	auto error = fopen_s( &filePtr,wavFileName,"rb" );
	assert( error == errno_t( 0 ) );

	// Read in the wave file header.
	WaveHeaderType waveFileHeader{};
	auto count = fread( &waveFileHeader, sizeof( waveFileHeader ), size_t( 1 ), filePtr );
	...
	auto result = pDirectSound->CreateSoundBuffer( &bufferDesc,&tempBuffer,NULL );
	assert( !FAILED( result ) );
	...
	count = fread( waveData, size_t( 1 ), size_t( waveFileHeader.dataSize ), filePtr );
